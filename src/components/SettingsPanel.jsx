...
import { useSettingsStore, useEmulatorStore } from '../store';
...
function SettingsPanel() {
  const {
    ...
    bluestacksPath, setBluestacksPath,
    scrcpyPath, setScrcpyPath,
    tracing, updateTracing,
    saveSettings
  } = useSettingsStore();
  
  const { checkMumuInstalled } = useEmulatorStore();
  ...
  const handleInstall = async () => {
    ...
    if (!status.droidrun.installed || !status.openai_like?.installed) {
  ...
  const allReady = status?.python?.installed && status?.droidrun?.installed && status?.adb?.installed && status?.openai_like?.installed;
      ...
      <section className="settings-section">
        ...
        <div className="settings-item">
          <div className="settings-item-info">
            <FolderOpen size={18} />
            <div>
              <span className="settings-item-label">MuMu Emulator</span>
              <span className="settings-item-desc">Thư mục nx_main của MuMu</span>
            </div>
          </div>
          <input
            type="text"
            className="settings-input"
            placeholder="C:\NetEase\MuMu\nx_main"
            value={emulatorPath}
            onChange={(e) => {
            ...
            onBlur={async () => {
              await saveSettings();
              // Refresh MuMu check after path change
        ...
        <div className="settings-item">
          <div className="settings-item-info">
            <FolderOpen size={18} />
            <div>
              <span className="settings-item-label">BlueStacks</span>
              <span className="settings-item-desc">Thư mục BlueStacks_nxt</span>
            </div>
          </div>
          <input
            type="text"
            className="settings-input"
            placeholder="C:\Program Files\BlueStacks_nxt"
            value={bluestacksPath || ''}
            onChange={(e) => {
              setBluestacksPath(e.target.value);
            }}
            onBlur={() => saveSettings()}
          />
        </div>
        ...
        <div className="settings-item">
          <div className="settings-item-info">
            <Monitor size={18} />
            <div>
              <span className="settings-item-label">Scrcpy</span>
              <span className="settings-item-desc">Thư mục chứa scrcpy.exe (để xem màn hình thiết bị)</span>
            </div>
          </div>
          <input
            type="text"
            className="settings-input"
            placeholder="D:\scrcpy-win64-v3.3.4"
            value={scrcpyPath || ''}
            onChange={(e) => {
              setScrcpyPath(e.target.value);
            }}
            onBlur={() => saveSettings()}
          />
        </div>
      ...
      <section className="settings-section">
        ...
        {status ? (
          <div className="settings-prerequisites">
            ...
            <div className={`settings-prereq-item ${status.openai_like?.installed ? 'ready' : ''}`}>
              <div className="settings-prereq-icon">
                {status.openai_like?.installed ? <Check size={14} /> : <AlertCircle size={14} />}
              </div>
              <span>OpenAI-Like LLM</span>
            </div>
